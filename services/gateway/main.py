...
5. ТЕХНИЧЕСКО ИЗСЛЕДВАНЕ (80-120 реда) - НАЙ-ВАЖНАТА СЕКЦИЯ!
   - 5.1 Определяне на скоростта (подробни изчисления):
     * Формула: u = √(2·μ·g·σ) - обяснете всяка променлива
     * Покажете изчислението стъпка по стъпка
     * Резултат с единици (m/s → km/h)
   - 5.2 Анализ по метода Momentum 360:
     * Входни данни: маси m₁, m₂, ъгли α, β
     * Формула за запазване на импулса: m₁V₁ + m₂V₂ = m₁u₁ + m₂u₂
     * Векторно разлагане по X и Y
     * Изчислени скорости V₁ и V₂
   - 5.3 Delta-V анализ:
     * ΔV = промяна на скоростта при удара
     * Оценка на тежестта на удара
   - 5.4 Възможност за предотвратяване:
     * Опасна зона
     * Спирачен път
     * Време за реакция
