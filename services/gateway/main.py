...
async def get_dual_ate_context(
    ...
    """
    # Query for legal requirements from Naredba 24 (including labor norms for damaged parts)
    parts_text = " ".join(damaged_parts[:5]) if damaged_parts else ""
    naredba_query = f"наредба 24 изисквания автотехническа експертиза нормочасове труд овехтяване {parts_text}"
...
══ НОРМИ ЗА ТРУД (по Наредба 24, Глава 3) ══
Часова ставка: {hourly_rate} лв/час
Примерни нормочасове (ако не са в данните):
• Смяна на бампер: 1.5-2.5 ч    • Смяна на калник: 2-3 ч
• Смяна на капак: 1-2 ч          • Смяна на врата: 3-5 ч
• Смяна на фар: 0.5-1 ч          • Смяна на огледало: 0.5-1 ч
• Боядисване на детайл: 2-4 ч   • Локално боядисване: 1-2 ч
...
6. ОЦЕНКА НА ЩЕТИТЕ (60 реда) - по Наредба 24
   ...
   - Списък повредени части с цени (от данните или реалистични: капак 400-800, бампер 200-500, фар 150-400 лв)
   ...
   - Труд: нормочасове × {hourly_rate} лв/час (използвайте таблицата с норми)
   - Боядисване: часове × {hourly_rate} лв/час (2-4 ч на детайл)
   - ОБЩА СТОЙНОСТ РЕМОНТ = части + труд + боядисване
